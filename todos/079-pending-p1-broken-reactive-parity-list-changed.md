...
priority: p1
issue_id: "079"
tags: ["agent-native", "backend", "mcp"]
dependencies: []
---

# Broken Reactive Parity (list_changed events not dispatched)



## Problem Statement

mcp-router fails to translate internal tool_list_changed events into standard MCP JSON-RPC notifications/tools/list_changed for active agent sessions.
...
mcp-server-manager.ipc.ts emits the internal event, but aggregator-server.ts and mcp-http-server.ts lack transport listeners to broadcast the notification.
