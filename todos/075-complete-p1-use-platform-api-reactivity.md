...
# React/Zustand Reactive Breakage in usePlatformAPI Hook



## Problem Statement

The usePlatformAPI hook fails to trigger a re-render when the workspace or auth token changes.
...
Zustand selector equality check sees that the function reference state.getPlatformAPI has not changed. Found in apps/electron/src/renderer/platform-api/hooks/use-platform-api.ts.
...
**Notes:** Updated usePlatformAPI hook to react to workspace/auth changes via memoized dependencies in /Users/robdezendorf/Documents/GitHub/mcp-router/apps/electron/src/renderer/platform-api/hooks/use-platform-api.ts.
