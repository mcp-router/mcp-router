...
# Unbounded Cache Misses in Tool Routing (Performance/DoS Risk)



## Problem Statement

The RequestHandlers class caches tool names to server names but does not cache negative lookups, causing full broadcast queries to all servers on every request for a non-existent tool.
...
Found in apps/electron/src/main/modules/mcp-server-runtime/request-handlers.ts lines 826-841.
