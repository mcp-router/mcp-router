...
export class ElectronPlatformAPI implements PlatformAPI {
  ...
  addMcpServer = (serverConfig: any) => window.electronAPI.addMcpServer(serverConfig);
  ...
  createAgent = (agentConfig: any) => window.electronAPI.createAgent(agentConfig);
  updateAgent = (id: string, config: any) => window.electronAPI.updateAgent(id, config);
