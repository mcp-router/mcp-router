...
export interface AppAPI {
  ...
  updateServerAccess(appName: string, serverIds: string[]): Promise<McpAppsManagerResult>;
  unifyConfig(appName: string): Promise<McpAppsManagerResult>;
  
  // Token management
  tokens: {
    updateScopes(tokenId: string, scopes: TokenScope[]): Promise<McpAppsManagerResult>;
